...
* `Port::Join(...)` materialises a named aggregation buffer that can operate in
  ...
  parameter or an inline `dim=`/`axis=` attribute.[code: join port helpers](../../../src/common/graph.hpp#L116-L220)
...
* `LinkParams::enable_graph_capture` turns CUDA graphs on for the compiled
  wiring plan (no effect when the spec list is empty).[code: graph capture flag](../../../src/core.hpp#L788-L803)
* `LinkParams::inputs` is an alias → index table for addressing multiple exposed
  inputs.
* `LinkParams::outputs` performs the same role for outputs. [code: output aliases](../../../src/core.hpp#L779-L803)
...
present even when the maps are left empty. [code: alias table defaults](../../../src/core.hpp#L801-L862) Aliases are
...
caught early. [code: alias validation](../../../src/core.hpp#L967-L1012)
...
configured dimension.[code: Join policy runtime](../../../src/core.hpp#L1891-L1916)
...
opting into CUDA graph capture for the final plan.[exemple: CIFAR routing example](../../../test/classification/images/cifar10.cpp#L144-L165)
