...
namespace Thot::Evaluation::Details::Timeseries {
    struct Descriptor { };
                                ...
                                const Options& options) -> Report
    {
        ...
        auto process_batch = [&](torch::Tensor prediction_tensor, Thot::StreamingBatch batch) {
            ...
            if (!preds.device().is_cpu()) {
                if (non_blocking_transfers) {
                    auto deferred = Thot::DeferredHostTensor::from_tensor(preds, /*non_blocking=*/true);
            ...
            if (!target_cpu.device().is_cpu()) {
                if (non_blocking_transfers) {
                    auto deferred = Thot::DeferredHostTensor::from_tensor(target_cpu, /*non_blocking=*/true);
