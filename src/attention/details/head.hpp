    ...
    public:
        ...
        explicit MultiHeadAttentionImpl(MultiHeadAttentionOptions options) : options_(std::move(options)) {
            ...
            if (options_.embed_dim <= 0) {
                throw std::invalid_argument("Multi-head attention requires a positive embedding dimension.");
            }
            if (options_.embed_dim % options_.num_heads != 0) {
                throw std::invalid_argument("Embedding dimension must be divisible by the number of heads.");
            }

            const auto embed_dim = options_.embed_dim;

            q_proj_ = register_module("q_proj", torch::nn::Linear(torch::nn::LinearOptions(embed_dim, embed_dim).bias(options_.bias)));
            k_proj_ = register_module("k_proj", torch::nn::Linear(torch::nn::LinearOptions(embed_dim, embed_dim).bias(options_.bias)));
            v_proj_ = register_module("v_proj", torch::nn::Linear(torch::nn::LinearOptions(embed_dim, embed_dim).bias(options_.bias)));
            out_proj_ = register_module("out_proj", torch::nn::Linear(torch::nn::LinearOptions(embed_dim, embed_dim).bias(options_.bias)));
                              ...
                              const torch::Tensor& key_padding_mask = {})
        {
            ...
            auto q_projected = q_proj_->forward(q).contiguous().view({batch_size, target_len, options_.num_heads, head_dim}).transpose(1, 2);
            auto k_projected = k_proj_->forward(k).contiguous().view({batch_size, source_len, options_.num_heads, head_dim}).transpose(1, 2);
            auto v_projected = v_proj_->forward(v).contiguous().view({batch_size, source_len, options_.num_heads, head_dim}).transpose(1, 2);

            auto attn_output = kernel_->forward(q_projected, k_projected, v_projected, attn_mask, key_padding_mask);
        ...
        [[nodiscard]] const MultiHeadAttentionOptions& options() const noexcept { return options_; }
