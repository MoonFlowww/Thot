...
template <typename T> __forceinline__ __device__ T ro(const T *p) {
...
namespace cuda::attentions {
    ...
    __global__ void mha_scaled_dot_product(const float *__restrict__ Q, const float *__restrict__ K,
                           const float *__restrict__ V, float *__restrict__ softmax,
                           float *__restrict__ context, int batch_size, int seq_len,
    ...
    void launchMHAForward(const float *input, const float *Wq, const float *Wk,
                          const float *Wv, const float *Wo, float *Q, float *K,
                          float *V, float *softmax, float *context, float *output,
        ...
        const int gridSize =
            (int)std::min<int64_t>((N + blockSize - 1) / blockSize, 65535);

        mha_linear<<<gridSize, blockSize, 0, stream>>>(input, Wq, Q, total_tokens,
                                                       embed_dim);
        mha_linear<<<gridSize, blockSize, 0, stream>>>(input, Wk, K, total_tokens,
                                                       embed_dim);
        mha_linear<<<gridSize, blockSize, 0, stream>>>(input, Wv, V, total_tokens,
                                                       embed_dim);
        CUDA_CHECK(cudaGetLastError());

        dim3 grid_sd(batch_size, num_heads);
        mha_scaled_dot_product<<<grid_sd, 1, 0, stream>>>(
            Q, K, V, softmax, context, batch_size, seq_len, num_heads,
            embed_dim / num_heads);
        CUDA_CHECK(cudaGetLastError());

        mha_linear<<<gridSize, blockSize, 0, stream>>>(context, Wo, output,
                                                       total_tokens, embed_dim);
        CUDA_CHECK(cudaGetLastError());
    }

    void launchMHABackward(const float *grad_output, float *grad_input,
                           float *grad_Wq, float *grad_Wk, float *grad_Wv,
                           float *grad_Wo, int total_tokens, int embed_dim,
                           cudaStream_t stream) {
